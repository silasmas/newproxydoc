<section class="single-product-wrap bg-light-primary100">
    <div class="container">
        <div class="row no-gutters">
            @if (Cart::count() <= 0) <div class="col-6 margin-b-30r">
        ...
        <div class="col-xl-7 col-lg-7 col-12 margin-b-30r">
            <div class="appointment-box-layout1 pt-0">
                <div class="widget widget-categories">
                    ...
                    <ul>
                        ...
                        <li>
                            <a href="{{ route('detailProdui', ['id' => $c->model->id]) }}">
                                ...
                                <label style="margin-left: 30px">{{ $c->model->monaie . '' . $c->subtotal() }}</label>

                                <span>
                                    <div class="cart-trash" x-data="{id:'{{ $c->rowId }}'}">
                                        <form @submit.prevent="$wire.deletConfirm(id)">

                                            <button type="submit" wire:loading.delay.attr='disabled'
                    ...
                    <hr>
                    <div class="row align-items-start mt-4" x-data="{livraison:false}"
                        x-init="$watch('livraison', value => console.log(value))">
                        <div class="col-lg-9">
                            <small class='text-danger'>La livraison à domicile est accéptée pour les achats supérieure à
                                <b>10$</b></small>
                        </div>
                        <div class="col-lg-3 d-flex justify-content-end">
                            <button class="btn btn-zone" data-toggle="modal" data-target="#modal-zone">Zone de
                                livraison</button>
                        </div>
                        @if (Cart::total() >= 10)
                        <div class="col-12 form-group carte mt-4">
                            <label for="">Voulez-vous être livré à domicile?</label><br>
                            <div class="form-check form-switch">
                                <input class="form-check-input " type="checkbox" x-bind:value="livraison"
                                    x-model="livraison" x-on:click="livraison=true" id="flexCheckDefault" role="switch">
                                <label class="form-check-label" x-text="livraison?'OUI':'NON'"
                                    for="flexCheckDefault"></label>
                            </div>

                            {{-- <select class="select2 comm" name="channels" onchange="switch_livraisone(this.value)"
                                id='livraisonChoice'>
                        ...
                        <div id="commune" class="col-12 form-group commune" x-show='livraison'>
                    ...
                    <form action="{{ url('achatProduit') }}" method="POST" data-parsley-validate>
                        ...
                        <div class="row gutters-15">
                            <div class="col-md-12 form-group">
                                <input type="email" placeholder="E-mail *" class="form-control" name="email"
                                    id="form-email" value="{{ old('email') }}" data-error="E-mail field is required"
                                    data-parsley-trigger="change" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="col-md-6 form-group">
                                <input type="text" placeholder="Votre Nom*" class="form-control" name="name" id="name"
                                    value="{{ old('name') }}" data-error="Champ obligatoire" data-parsley-minlength="3"
                                    data-parsley-trigger="change" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="col-md-6 form-group">
                                <input type="text" placeholder="Votre prenom*" class="form-control" name="prenom"
                                    id="prenom" data-error="Champ obligatoir" required value="{{ old('prenom') }}"
                        ...
                        <div class="row gutters-15">
                            <div>
                                ...
                                <div class="col-12 form-group">
                                    {{-- <input type="text" class="form-control" name="abonnement_id"
                                        value="{{ $ab->id }}" id="abonnement_id" data-error="Name field is required"
                                        required>
                                    <div class="help-block with-errors"></div> --}}
                                </div>
                                <div class="col-12 form-group">
                                    {{-- <input type="text" class="form-control" name="prix" value="{{ Cart::total() }}"
                                        id="prix">
                                    <div class="help-block with-errors"></div> --}}
                                </div>
                                <div class="col-12 form-group">
                            ...
                            <section class="col-12 " id="carte" style="display: none">
                                ...
                                <div class="col-12 form-group">
                                    <textarea placeholder="Votre adresse" class="textarea form-control carte2"
                                        name="customer_address" id="customer_address" rows="5" cols="20">
                ...
                <nav>
                    <div class="nav nav-tabs nav-zone" id="nav-tab" role="tablist">
                        <button class="nav-link active" id="nav-home-tab" data-toggle="tab" data-target="#nav-home"
                            type="button" role="tab" aria-controls="nav-home" aria-selected="true">Zone A</button>
                        <button class="nav-link" id="nav-profile-tab" data-toggle="tab" data-target="#nav-profile"
                            type="button" role="tab" aria-controls="nav-profile" aria-selected="false">Zone B</button>
                        <button class="nav-link" id="nav-contact-tab" data-toggle="tab" data-target="#nav-contact"
                            type="button" role="tab" aria-controls="nav-contact" aria-selected="false">Zone C</button>
                    </div>
                </nav>
